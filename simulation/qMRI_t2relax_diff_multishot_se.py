...
# ================================================================================
# b=0 with a single direction gives no diffusion weighting — identical to the
# non-diffusion SE, but validates the DiffusionSEMultishotPulseqSeq class.
...
for te in TEs:
    ...
    # ============================================================================
    # Signal layout: 1 direction × N_shots × ETL × Nx = Ny × Nx samples.
    # With b_directions=1, the direction loop in build_seq runs once, so the signal
    # is simply reshaped to (Ny, Nx).
...
axs = axs.flatten()
fig.suptitle(
    f"Diffusion SE Multishot T2 relaxometry  (b=0, ETL={ETL}, {N_shots} shots/TE, "
    f"total TRs={len(TEs)*N_shots})"
)
...
plt.suptitle(
    f"Diffusion SE Multishot T2 maps (b=0, ETL={ETL}, {len(TEs)} TEs, {len(TEs)*N_shots} TRs)"
)
plt.tight_layout()
plt.show()
...
fig.suptitle(
    f"Diffusion SE Multishot T2 mapping (b=0) — ETL={ETL}  "
...
# %%
try:
    np.save(rf"{VOLUMES_DIR_PATH}\T2_multishot_se.npy", ims2[0]/1000) # save in seconds
    print("Saved T2 maps.")
