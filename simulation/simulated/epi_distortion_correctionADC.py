...
triple_bvals = np.unique(
    [int(f.split("b")[1].split("-")[0]) for f in all_bval_files if "blipup" in f]
)
...
for b_val in triple_bvals:
    ...
    bu_files = sorted(
        [f for f in all_bval_files if f.startswith(f"b{b_val}-") and "blipup" in f]
    )
    bd_files = sorted(
        [f for f in all_bval_files if f.startswith(f"b{b_val}-") and "blipdown" in f]
    )
    ...
    for te in tes:
        bu_te = np.array(
            [
                nib.load(os.path.join(BVAL_DIR, f)).get_fdata().squeeze()
                for f in bu_files
                if f"TE{te}" in f
            ]
        )
    ...
    trace_bd_path = os.path.join(
        BVAL_DIR, f"trace-DiffTripleSE-b{b_val}-blipdown.nii.gz"
    )
    nib.save(
        nib.Nifti1Image(trace_bu[:, :, np.newaxis].astype(np.float32), affine),
        trace_bu_path,
    )
    nib.save(
        nib.Nifti1Image(trace_bd[:, :, np.newaxis].astype(np.float32), affine),
        trace_bd_path,
    )
    ...
    loss_func = EPIMRIDistortionCorrection(
        data, 1000, 1e-7, regularizer=myLaplacian1D, PC=JacobiCG
    )
    ...
    ax[3].imshow(
        nib3.get_fdata().squeeze() - nib4.get_fdata().squeeze(), cmap="seismic"
    )
...
# ============================================================================
single_se_files = [
    f for f in os.listdir(BVAL_DIR) if f.endswith(".nii.gz") and f.startswith("DiffSE")
]
...
if single_se_files:
    single_bvals = np.unique(
        [int(f.split("b")[1].split("-")[0]) for f in single_se_files if "blipup" in f]
    )
    ...
    for b_val in single_bvals:
        ...
        bu_files = sorted(
            [f for f in single_se_files if f"b{b_val}-" in f and "blipup" in f]
        )
        bd_files = sorted(
            [f for f in single_se_files if f"b{b_val}-" in f and "blipdown" in f]
        )
    ...
    print(
        "Run qMRI_adc.py (with NIfTI saving) to generate DiffSE blip-up/down files first."
    )
...
if len(single_bu) > 0:
    adc_results["single_bu"] = create_adc_map(
        np.array(single_bu), single_bvals, method="nlls"
    )[0]
    adc_results["single_bd"] = create_adc_map(
        np.array(single_bd), single_bvals, method="nlls"
    )[0]
...
# ============================================================================
np.save(
    os.path.join(results_dir, "ADC_triple_se_distcorr_blipup.npy"),
    np.rot90(adc_results["triple_bu"], -1),
)
np.save(
    os.path.join(results_dir, "ADC_triple_se_distcorr_blipdown.npy"),
    np.rot90(adc_results["triple_bd"], -1),
)

if "single_bu" in adc_results:
    np.save(
        os.path.join(results_dir, "ADC_single_se_distcorr_blipup.npy"),
        np.rot90(adc_results["single_bu"], -1),
    )
    np.save(
        os.path.join(results_dir, "ADC_single_se_distcorr_blipdown.npy"),
