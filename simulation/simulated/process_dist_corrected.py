...
t2_blipup_paths = sorted(
    [i for i in os.listdir(rf'{t2_dir}') if i.endswith('blipup.nii.gz')],
    key=te_value
)
t2_blipdown_paths = sorted(
    [i for i in os.listdir(rf'{t2_dir}') if i.endswith('blipdown.nii.gz')],
    key=te_value
)
TEs = get_TEs(t2_blipup_paths)
print(f"Echo times (TEs) extracted: {TEs}")
print(f"Found {len(t2_blipup_paths)} blipup and {len(t2_blipdown_paths)} blipdown files.")
...
for i in range(niftis_bu_resampled.shape[1]):
    print(f"Processing blipup slice {i+1}/{niftis_bu_resampled.shape[1]}...", end="\r", flush=True)
...
for i in range(niftis_bd_resampled.shape[1]):
    print(f"Processing blipdown slice {i+1}/{niftis_bd_resampled.shape[1]}...", end="\r", flush=True)
    t2_map, s0_map = create_t2_map(niftis_bd_resampled[:, i, :, :], TEs)
...
# Save Output
# =================================
t2_blipdown_map = np.transpose(t2_blipup_map, (2,1,0))
t2_blipdown_map = np.transpose(t2_blipdown_map, (2,1,0))
