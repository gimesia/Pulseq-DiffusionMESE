...
adcSSE_blipup = np.load("brainmaps/ADC_SSEblipup.npy")
adcSSE_blipdown = np.load("brainmaps/ADC_SSEblipdown.npy")
adcMESE_blipup = np.load("brainmaps/ADC_MSEblipup.npy")
adcMESE_blipdown = np.load("brainmaps/ADC_MSEblipdown.npy")
adcRef = np.load("brainmaps/ADC_ref.npy")
adcmultishot_se = np.fliplr(np.load("brainmaps/ADC_multishot_se.npy"))
...
plot_ims = [
    [adcSSE_blipup, adcMESE_blipup, np.abs(adcMESE_blipup - adcSSE_blipup)],
    [adcSSE_blipdown, adcMESE_blipdown, np.abs(adcMESE_blipdown - adcSSE_blipdown)],
    # [t2SSE_distcorr_blipdown, t2MESE_distcorr_blipdown, np.abs(t2MESE_distcorr_blipdown - t2SSE_distcorr_blipdown)],
    [adcmultishot_se, adcRef, np.abs(adcmultishot_se - adcRef.squeeze())]
]

plot_titles: list[list[str]] = [
    ["ADC SSE Blip-Up", "ADC MESE Blip-Up", "Difference Blip-Up"],
    ["ADC SSE Blip-Down", "ADC MESE Blip-Down", "Difference Blip-Down"],
    # ["T2 SSE Dist-Corr", "T2 MSE Dist-Corr", "Difference Dist-Corr"],
    ["ADC Multishot SE", "ADC Reference", "Difference Multishot SE vs Ref"]
]
...
for i in range(rows):
    ...
    difference_img = fixed_img - warped_moving
    ...
    for j in range(3):
        fig.colorbar(ax[i, j].imshow(plot_ims[i][j]), ax=ax[i, j])
...
refs = [np.squeeze(adcRef), adcmultishot_se, adcSSE_blipup, adcSSE_blipdown, adcMESE_blipup, adcMESE_blipdown]
