...
mseTEs = np.unique(
    [int(f.split("-")[-2].replace(".nii.gz", "").replace("TE", "")) for f in filenames]
)
...
# %%
relaxometry_results = {
    "mse_bu": create_t2_map(np.squeeze(mse_imgs_bu), mseTEs, method="nlls"),
...
ax = ax.flatten()
titles = [
    "MSE (blipup) T2 Map (NLLS Fit)",
    "SSE (blipup) T2 Map (NLLS Fit)",
    "MSE (blipdown) T2 Map (NLLS Fit)",
    "SSE (blipdown) T2 Map (NLLS Fit)",
]
for i, key in enumerate(relaxometry_results.keys()):
    im = ax[i].imshow(np.rot90(relaxometry_results[key][0], -1), cmap="viridis")
    ax[i].set_title(titles[i])
fig.suptitle(
    f"Distortion-Corrected T2 Maps (NLLS Fit) for MSE and SSE Blip-Up/Blip-Down Images"
)
plt.tight_layout()
# save results
...
np.save(
    os.path.join(results_dir, "T2_MSE_distcorr_blipup.npy"),
    np.rot90(relaxometry_results["mse_bu"][0] / 1000, -1),
)  # save AP and in s
np.save(
    os.path.join(results_dir, "T2_SSE_distcorr_blipup.npy"),
    np.rot90(relaxometry_results["sse_bu"][0] / 1000, -1),
)  # save AP and in s
np.save(
    os.path.join(results_dir, "T2_MSE_distcorr_blipdown.npy"),
    np.rot90(relaxometry_results["mse_bd"][0] / 1000, -1),
)  # save AP and in s
np.save(
    os.path.join(results_dir, "T2_SSE_distcorr_blipdown.npy"),
    np.rot90(relaxometry_results["sse_bd"][0] / 1000, -1),
)  # save AP and in s
