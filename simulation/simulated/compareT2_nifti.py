...
SUBJECT_ID = 0
subjects = np.unique([f.split('-')[1] for f in os.listdir("volumes/") if f.startswith("brainweb-subj")])
...
t2SSE_blipup    = load_nii(f"volumes/brainweb-{subject}-T2_NLLS_t2_sse_blipup_volume.nii.gz")
t2SSE_blipdown  = load_nii(f"volumes/brainweb-{subject}-T2_NLLS_t2_sse_blipdown_volume.nii.gz")
...
t2multishot_se  = load_nii(f"volumes/brainweb-{subject}-T2_NLLS_t2_multishot_volume.nii.gz")
...
if REGISTER:
    ...
    t2SSE_blipup_reg    = _reg(f"volumes/brainweb-{subject}-T2_NLLS_t2_sse_blipup_volume.nii.gz")
    t2SSE_blipdown_reg  = _reg(f"volumes/brainweb-{subject}-T2_NLLS_t2_sse_blipdown_volume.nii.gz")
    t2MESE_blipup_reg   = _reg(f"volumes/brainweb-{subject}-T2_NLLS_t2_triple_blipup_volume.nii.gz")
    t2MESE_blipdown_reg = _reg(f"volumes/brainweb-{subject}-T2_NLLS_t2_triple_blipdown_volume.nii.gz")
    t2multishot_se_reg  = _reg(f"volumes/brainweb-{subject}-T2_NLLS_t2_multishot_volume.nii.gz")
...
# =================================================================================
pairs = [
    (t2SSE_blipup_reg,   t2MESE_blipup_reg,   "T2 SSE Blip-Up (registered)",   "T2 MESE Blip-Up (registered)",   "Difference Blip-Up"),
    (t2SSE_blipdown_reg, t2MESE_blipdown_reg, "T2 SSE Blip-Down (registered)", "T2 MESE Blip-Down (registered)", "Difference Blip-Down"),
    (t2multishot_se_reg, t2Ref_reg,            "T2 Multishot SE (registered)",  "T2 Reference",                   "Difference Multishot SE vs Ref"),
]
...
# =================================================================================
refs      = [t2Ref_reg, t2multishot_se_reg, t2SSE_blipup_reg, t2SSE_blipdown_reg, t2MESE_blipup_reg, t2MESE_blipdown_reg]
...
ax[0, 0].axis('off')
ax[0, 0].text(0.5, 0.5, "T2", fontsize=36, fontweight='bold',
...
fig_masks, axs_masks = plt.subplots(1, len(tissue_names_masks), figsize=(6 * len(tissue_names_masks), 6))
...
for t in range(n_tissues):
    for i in range(n):
        for j in range(n):
            if i != j:
                mae_matrices[t, i, j] = mae_tissue(refs[i], refs[j], tissue_masks_arr[t]) * 1000
...
ax_mean.set_title("Mean T2 per tissue by acquisition\n(SSE / MSE averaged over blip-up and blip-down)")
ax_mean.legend()
plt.tight_layout()
plt.show()
