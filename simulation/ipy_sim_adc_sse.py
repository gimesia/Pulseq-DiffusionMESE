...
ECHO_IMAGES_DIR_PATH = _paths.SIMULATION_DIR / "simulated" / "diff_img"
...
for te in TE_VALUES:
    for b_value in b_values:
        ...
        traj = np.stack([kx_norm, ky_norm], axis=-1)  # (n_dirs * Ny * Nx, 2)

        samples_per_dir = Ny * Nx
        direction_trajectories = np.array([
            traj[d * samples_per_dir : (d + 1) * samples_per_dir]
        ...
        for d in range(n_dirs):
            nufft_op = get_operator(
                ...
                samples=direction_trajectories[d],
                shape=img_size,
                n_coils=1,
                density=True,
            )
            kspace = signal[d * samples_per_dir : (d + 1) * samples_per_dir].numpy()
            sig = torch.from_numpy(kspace).to(torch.complex64)
...
for i, adc in enumerate([adc_nlls, adc_ll]):
    im = axs[i].imshow(np.rot90(adc, -1) * 1e3, cmap=cm.turku)
...
ims_data = [
    (np.rot90(adc_nlls, -1) * 1e3, "NLLS ADC", cm.turku),
    (np.rot90(adc_ll, -1) * 1e3, "Log-Linear ADC", cm.turku),
    (D_ref, "Reference D map", cm.turku),
]
