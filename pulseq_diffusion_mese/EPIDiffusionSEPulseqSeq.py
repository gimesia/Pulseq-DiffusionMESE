...
class EPIDiffusionSEPulseqSeq(PulseqSeq):
    ...
    def __init__(
        ...
        # --- PulseqSeq base init ---
        self._init_logging(
            logger or logging.getLogger("EPIDiffusionSEPulseqSeqV4"),
            name,
            system_type,
            save_dir,
        )
    ...
    def _run_epi_fit_loop(self):
        ...
        epi_fit = False
        fit_step = (
            0.05  # 5 % PF reduction per retry — coarse enough to converge quickly
        )
        while not epi_fit:
    ...
    def init_message(self):
        ...
        self.logger.info(
            f"Initializing EPI Diffusion Spin Echo Pulseq Sequence: {self.name}"
        )
